...
import { buscarProveedores, ProveedorOption } from '../../services/proveedorService';

interface ProveedorAutocompleteProps {
    value: string;
    selectedName: string;
    onChange: (proveedorId: string, nombre: string) => void;
    error?: string;
}
...
const ProveedorAutocomplete: React.FC<ProveedorAutocompleteProps> = ({
    ...
    const [results, setResults] = useState<ProveedorOption[]>([]);
    ...
    const searchProveedores = useCallback(async (query: string) => {
        if (!query.trim()) {
            setResults([]);
        ...
        setLoading(true);
        try {
            const response = await buscarProveedores(query);
            setResults(response.proveedores);
    ...
    const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        debounceTimerRef.current = setTimeout(() => {
            searchProveedores(val);
        }, 300);
    };

    const handleSelect = (proveedor: ProveedorOption) => {
        setSearch(proveedor.nombre || '');
        setShowDropdown(false);
        setResults([]);
        onChange(proveedor.id_proveedor, proveedor.nombre || '');
    };
